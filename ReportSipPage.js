...
import { ReportSipTable } from "./ReportSipTable";
...
export const ReportSipPage = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.payload.status === 200) {
        ...
        link.download = "Report_Surat_Instruksi_Pengiriman.xlsx";
        link.click();
      } else {
        // Corrected the syntax here
        const action = await showErrorDialog(response.payload.data.message);
  ...
  return loading ? (
    ...
    <Card>
      <CardHeader title="Report Surat Instruksi Pengiriman">
        <CardHeaderToolbar>
